...
sed -i 's/MINIO_ACCESS_KEY/'"$MINIO_KAFKA_ACCESS_KEY"'/' oniondownloader.yaml
sed -i 's/MINIO_SECRET_KEY/'"$MINIO_KAFKA_SECRET_KEY"'/' oniondownloader.yaml
sed -i 's/MINIO_API_ENDPOINT/'"$MINIO_API_ENDPOINT"'/' oniondownloader.yaml
kubectl apply -f ns.yaml
kubectl apply -f oniondownloader.yaml
sed -i 's/'"$PRIVATE_DOCKER_REGISTRY"'/PRIVATE_DOCKER_REGISTRY/' oniondownloader.yaml
sed -i 's/'"$MINIO_KAFKA_ACCESS_KEY"'/MINIO_ACCESS_KEY/' oniondownloader.yaml
sed -i 's/'"$MINIO_KAFKA_SECRET_KEY"'/MINIO_SECRET_KEY/' oniondownloader.yaml
sed -i 's/'"$MINIO_API_ENDPOINT"'/MINIO_API_ENDPOINT/' oniondownloader.yaml